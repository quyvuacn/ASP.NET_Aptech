window.onload = function () {
	...
	HSCore.components.HSDropzone.init(".js-dropzone", {
		init: function () {
			let form = $(this.previewsContainer)
			let actionTarget = form.attr("action")
			this.options.url = actionTarget
			this.options.parallelUploads = 4
	...
	// INITIALIZATION OF DATATABLES
	// =======================================================
	HSCore.components.HSDatatables.init($("#datatableProductDetails"), {
		select: {
			style: "multi",
			selector: 'td:first-child input[type="checkbox"]',
			classMap: {
	...
	new HSAddField(".js-add-field", {
		addedField: (field) => {
			new HSQuantityCounter(
				field.querySelector(
					".js-quantity-counter-dynamic",
				),
			)
		},
	})
